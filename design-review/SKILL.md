...
REPORT_DIR=$GSTACK_PROJECTS/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
```
$GSTACK_PROJECTS/$SLUG/designs/design-audit-{YYYYMMDD}/
...
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p $GSTACK_PROJECTS/$SLUG
```
Write a one-line summary to `$GSTACK_PROJECTS/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
