...
_PREV=$(find $GSTACK_PROJECTS/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
ls $GSTACK_PROJECTS/$SLUG/*office-hours* 2>/dev/null | head -5
```
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find $GSTACK_PROJECTS/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
_DESIGN_DIR=$GSTACK_PROJECTS/$SLUG/designs/<screen-name>-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `$GSTACK_PROJECTS/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
