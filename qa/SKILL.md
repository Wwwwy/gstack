...
1. **Project-scoped test plans:** Check `$GSTACK_PROJECTS/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
   ls -t $GSTACK_PROJECTS/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
```
Write to `$GSTACK_PROJECTS/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
